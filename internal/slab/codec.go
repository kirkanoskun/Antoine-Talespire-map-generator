...
func decode(code string, compressedLimit int) (*Slab, error) {
	if len(code) > MaxInputBytes {
		return nil, fmt.Errorf("Slab code exceeds 1 MiB")
	}
	code = strings.TrimSpace(code)
	if strings.HasPrefix(code, "talespire://") {
		return nil, fmt.Errorf("this is a Board link; paste a Slab code instead")
	}
	if strings.HasPrefix(code, "```") {
		if len(code) < 6 || !strings.HasSuffix(code, "```") {
			return nil, fmt.Errorf("unclosed Slab code fence")
		}
	...
	if err != nil {
		return nil, fmt.Errorf("invalid Slab base64: %w", err)
	}
	if len(data) > compressedLimit {
		return nil, fmt.Errorf("compressed Slab exceeds %d bytes; split it in TaleSpire", compressedLimit)
	}
	source := bytes.NewReader(data)
	gz, err := gzip.NewReader(source)
	if err != nil {
		return nil, fmt.Errorf("invalid Slab gzip: %w", err)
	}
	defer gz.Close()
	...
	if len(raw) > MaxDecodedBytes {
		return nil, fmt.Errorf("decompressed Slab exceeds 4 MiB")
	}
	if source.Len() != 0 {
		return nil, fmt.Errorf("unexpected data after Slab gzip stream")
	}
	return parse(raw)
}
...
func parse(raw []byte) (*Slab, error) {
	if len(raw) < 10 {
		return nil, fmt.Errorf("Slab header is truncated")
	}
	le := binary.LittleEndian
	if le.Uint32(raw) != 0xD1CEFACE {
		return nil, fmt.Errorf("invalid Slab signature")
	}
	if version := le.Uint16(raw[4:]); version != 2 {
		return nil, fmt.Errorf("unsupported Slab version %d; only v2 is supported", version)
	}
	n := int(le.Uint16(raw[6:]))
	if le.Uint16(raw[8:]) != 0 {
		return nil, fmt.Errorf("creatures are not supported in v2 Slabs")
	}
	offset := 10 + 20*n
	if len(raw) < offset {
		return nil, fmt.Errorf("Slab layout table is truncated")
	}
	...
	for i := range out.Layouts {
		...
		total += count
		if total > MaxAssets {
			return nil, fmt.Errorf("Slab exceeds %d assets", MaxAssets)
		}
	...
	for i := range out.Layouts {
		for j := range out.Layouts[i].Instances {
			...
			if p.Rotation >= 24 {
				return nil, fmt.Errorf("invalid Slab rotation %d", p.Rotation)
			}
...
func Encode(s *Slab) (string, error) {
	if s == nil || len(s.Layouts) > 65535 {
		return "", fmt.Errorf("invalid Slab layout count")
	}
	var raw bytes.Buffer
	write := func(v any) { _ = binary.Write(&raw, binary.LittleEndian, v) }
	write(uint32(0xD1CEFACE))
	write(uint16(2))
	write(uint16(len(s.Layouts)))
	write(uint16(0))
	total := 0
	for _, l := range s.Layouts {
		total += len(l.Instances)
		if len(l.Instances) > 65535 || total > MaxAssets {
			return "", fmt.Errorf("too many Slab assets")
		}
		raw.Write(l.ID[:])
		write(uint16(len(l.Instances)))
		write(l.Reserved)
	}
	for _, l := range s.Layouts {
	...
	if raw.Len() > MaxDecodedBytes {
		return "", fmt.Errorf("Slab exceeds decoded size limit")
	}
